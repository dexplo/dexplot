...
_NORMALIZE_ERROR_MSG = '`normalize` can only be None, "all", one of the values passed to ' \
                      ' the parameter names "agg", "groupby2", "row", "col", or a combination' \
                      ' of those parameter names in a tuple, if they are defined.'


class AggPlot(CommonPlot):

    COLUMN_PARAMS = ['agg', 'groupby', 'groupby2', 'groupby_row', 'groupby_col']

    def __init__(self, data, agg, aggfunc, groupby, groupby2, groupby_row, groupby_col, 
                 orientation, sort, wrap, figsize, title, sharex, sharey, xlabel, ylabel, 
                 xlim, ylim, xscale, yscale, kind):

        self.data = data
        self.agg = agg
        self.aggfunc = aggfunc
        self.groupby = groupby
        self.groupby2 = groupby2
        self.groupby_row = groupby_row
    ...
    def get_uniques(self):
        uniques = {}
        for param, vals in self.column_data.items():
            uniques[param] = np.sort(vals.dropna().unique())
        return uniques
    ...
    def set_single_plot_labels(self, ax):
        if self.kind in ('bar', 'line', 'box'):
            if self.orientation == 'v' and self.agg_kind != 'O':
            ...
            elif self.orientation == 'h' and self.agg_kind != 'O':
                if not (self.groupby or self.normalize):
                    ...
                    ax.set_xlabel(self.xlabel or self.agg)
            elif self.orientation == 'v' and self.agg_kind == 'O':
                pass

            if self.groupby2 and self.no_legend:
            ...
            if self.orientation == 'v':
                ...
                ax.set_ylabel(self.ylabel or self.agg)
            if (self.groupby or self.groupby2) and self.no_legend:
                ax.figure.legend(bbox_to_anchor=(1.01, .5), loc='center left')

    def set_figure_plot_labels(self, fig):
        label_fontsize = plt.rcParams['font.size'] * 1.5
        only_agg = not (self.groupby or self.groupby2)
        is_vert = self.orientation == 'v'
        is_blb = self.kind in ('bar', 'line', 'box')
        is_hk = self.kind in ('hist', 'kde')
        is_numeric = self.agg_kind != 'O'
        is_numeric_split = self.groupby2 and self.groupby and is_numeric
        is_numeric_split_groupby2 = (self.groupby2 or self.groupby) and is_numeric and is_hk
        is_cat_split = self.groupby2 and not is_numeric
        for ax in fig.axes:
        ...
        if is_numeric:
            if is_blb and is_vert:
                ...
                fig.text(-.02, .5, self.agg, rotation=90, fontsize=label_fontsize,
                         ha='center', va='center')
        if (is_numeric_split or is_cat_split or is_numeric_split_groupby2) and not fig.legends:
    ...
    def plot_groupby2_agg(self, ax, data):
        if self.agg_kind == 'O':
            tbl = pd.crosstab(data[self.agg], data[self.groupby2])
            if self.normalize is not None:
                tbl = tbl.stack().reset_index()
                tbl = self.do_normalization(tbl, data)
                tbl = tbl.iloc[:, 0].unstack()
            tbl = tbl.reindex(index=self.all_aggs, columns=self.all_groupby2s)
            self.plot_func(ax, tbl)
        else:
            if self.kind in ('box', 'hist', 'kde'):
                data_array = []
                g = data.groupby(self.groupby2)
                for groupby2 in self.all_groupby2s:
                    if groupby2 in g.groups and len(g.groups[groupby2]) > 0:
                        data_array.append(g.get_group(groupby2)[self.agg].values)
                    else:
                        data_array.append([])
                bp = self.plot_func(ax, data_array, labels=self.all_groupby2s)
                if self.kind == 'box':
                    patches = bp['boxes']
                    for i, patch in enumerate(patches):
                        patch.set(facecolor=plt.cm.tab10(i))
                    if self.no_legend:
                        ax.figure.legend(handles=patches, labels=self.all_groupby2s.tolist(),
                                         bbox_to_anchor=(1, .5), loc='center left')
                        self.no_legend = False
            else:
                final_data = data.pivot_table(columns=self.groupby2, values=self.agg, aggfunc=self.aggfunc)
                final_data = final_data.reindex(columns=self.all_groupby2s)
    ...
    def plot_groupby_groupby2_agg(self, ax, data):
        # might need to refactor to put in all_groupby2s, all_groups, all_aggs
        if self.kind in ('bar', 'line'):
            tbl = data.pivot_table(index=self.groupby, columns=self.groupby2,
                                   values=self.agg, aggfunc=self.aggfunc)
            tbl = tbl.reindex(index=self.all_groupbys, columns=self.all_groupby2s)
            self.plot_func(ax, tbl)
        else:
            ...
            ax.figure.legend(handles=box_plots, labels=groupby2_labels,
    ...
    def plot_row_or_col(self, axes):
        ...
        for i, (ax, (val, sub_df)) in enumerate(zip(axes_flat, g)):
            if not (self.groupby or self.groupby2):
                self.plot_only_agg(ax, sub_df)
            elif self.groupby2 and not self.groupby:
                self.plot_groupby2_agg(ax, sub_df)
            elif not self.groupby2 and self.groupby:
                self.plot_groupby_agg(ax, sub_df)
            elif self.groupby2 and self.groupby:
                self.plot_groupby_groupby2_agg(ax, sub_df)
            ax.set_title(val)
...
s = \
    ...
    `normalize` by setting it to one of the strings 'agg', 'groupby2', 'row',
    'col', or 'all'.
    ...
    groupby2: str
        ...
        Each unique value in the `groupby2` column forms a new group.
    ...
    normalize: str, tuple
        ...
        normalize over with the strings 'agg', 'groupby2', 'row', 'col', or 'all'.
        Use a tuple of these strings to normalize over two or more of the
        above. For example, use ('groupby2', 'row') to normalize over the
        `groupby2`, `row` combination.
...
def line(data, agg, aggfunc, groupby=None, groupby2=None, groupby_row=None, groupby_col=None, 
        ...
        ap = AggPlot(data, agg, aggfunc, groupby, groupby2, groupby_row, groupby_col, 
                     orientation, sort, wrap, figsize, title, sharex, 
                     sharey, xlabel, ylabel, xlim, ylim, xscale, yscale, kind='line')

        if ap.agg_kind == 'O':
            raise ValueError('Cannot do line plot when the `agg` variable is object/categorical')

        fig, ax = ap.create_figure()
        if ap.single_plot:
            if ap.groupby is None and ap.groupby2 is None:
                value = data[ap.agg].agg(ap.aggfunc)
                ax.plot(value, marker='o')
            else:
                df_grouped = data.pivot_table(index=ap.groupby, columns=groupby2, 
                                              values=agg, aggfunc=aggfunc)
                x = df_grouped.index
                for col in df_grouped.columns:
                    y = df_grouped[col].values
                    ax.plot(x, y, marker='o', label=col)
                if ap.groupby is None:
                    ax.set_xticks([])
                        
            ax.legend(title=ap.groupby2)
            ax.set_xlabel(ap.groupby)
            ax.set_ylabel(agg)
            return ax
...
#             if self.groupby and self.groupby2:
#                 raise NotImplementedError('When plotting a "hist" or "kde", you can set at most '
#                                           'one of `groupby` or `groupby2` but not both')

# self.validate_kde_hist()


# must validate for specific plot
# def validate_agg_kind(self):
#         self.agg_data = self.data[self.agg]
#         self.agg_kind = self.agg_data.dtype.kind
