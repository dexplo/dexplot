...
class TestAgg:
    ...
    def test_function(self):
        dxp.scatter(x='neighborhood', y='price', data=airbnb, aggfunc=np.median)
...
class TestSort:
    ...
    def test_lex_asc(self):
        fig = dxp.scatter(x='neighborhood', y='price', data=airbnb, aggfunc='median')
    ...
    def test_lex_desc(self):
        fig = dxp.scatter(x='neighborhood', y='price', data=airbnb, aggfunc='median', sort_values='lex_desc')
        ticklabels = [t.get_text() for t in fig.axes[0].get_xticklabels()]
        correct = sorted(ticklabels, reverse=True)
        assert ticklabels == correct

    def test_asc_values(self):
        fig = dxp.scatter(x='neighborhood', y='price', data=airbnb, aggfunc='median', sort_values='asc')
        ticklabels = [t.get_text() for t in fig.axes[0].get_xticklabels()]
    ...
    def test_desc_values(self):
        fig = dxp.scatter(x='neighborhood', y='price', data=airbnb, aggfunc='median', sort_values='desc')
        ticklabels = [t.get_text() for t in fig.axes[0].get_xticklabels()]
        ticklabels = [label.replace('\n', ' ') for label in ticklabels]
        values = [p.get_height() for p in fig.axes[0].patches]
...
class TestHorizontal:
    ...
    def test_horiz(self):
        dxp.scatter(x='price', y='neighborhood', data=airbnb, aggfunc='median', orientation='h')


class TestSplit:

    def test_split(self):
        dxp.scatter(x='neighborhood', y='price', data=airbnb, aggfunc='median', split='superhost')
